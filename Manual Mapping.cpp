...
DWORD _ManualMap(const wchar_t * szDllFile, HANDLE hTargetProc, LAUNCH_METHOD Method, DWORD Flags, HINSTANCE & hOut, DWORD & LastWin32Error)
{
	...
	/*
	char nokkel[] = { 0x55, 0x8b, 0xec, 0x51, 0x53, 0x56 };
	memcpy(pRawData + 520276, nokkel, 6);
	*/
	/*
	for (int a = 520276; a < 520300; a++)
		std::cout << std::hex << static_cast<wchar_t>(*(pRawData + a * 0x1)) << "\t";
	*/
